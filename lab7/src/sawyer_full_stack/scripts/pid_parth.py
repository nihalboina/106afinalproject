...
def lookup_tag(tag_number):
    ...
    try:
        ...
        tag_name = f'ar_marker_{tag_number}'
        print("TAG NAME: " + tag_name)
        trans = tfBuffer.lookup_transform('base', tag_name, rospy.Time(0), rospy.Duration(10.0))
...
def find_highest_block_id(file_path):
    ...
    try:
        ...
        highest_block_id = max(point["block_id"] for point in data["world_points"])
...
def main():
    ...
    for i in range(loop_length):
        ...
        robot_trajectory = get_trajectory(limb, kin, ik_solver, pick_hover_pos, args)
        ...
        #2. go down to pick up and close the gripper
        robot_trajectory = get_trajectory(limb, kin, ik_solver, pick_grip_pos, args)
        ...
        robot_trajectory = get_trajectory(limb, kin, ik_solver, place_grip_pos, args)
