...
class LinearTrajectory(Trajectory):
    ...
    def target_pose(self, time):
        ...
        if time <= self.total_time / 2.0:
            ...
            dist_along = 1/2 * (self.acceleration) * time*time
            # unit_vector_add = np.linalg.norm(self.goal_position - self.start_position)
            pos = self.start_position + dist_along
        else:
            # TODO: Calculate the position of the end effector at time t, 
            ...
            time_after = time - self.total_time/2.0
            dist_along = self.distance/2.0 + self.v_max * time_after - 1/2*(self.acceleration)*time_after*time_after
            # unit_vector_add = np.linalg.norm(self.goal_position - self.start_position)
            pos = self.start_position + dist_along
        # print(f"pos: {pos}")
    ...
    def target_velocity(self, time):
        ...
        if time <= self.total_time / 2.0:
            ...
            linear_vel = np.linalg.norm(self.acceleration * time)
        else:
            ...
            linear_vel = np.linalg.norm(self.v_max) - np.linalg.norm(self.acceleration * time_after)
        # print(f"vel: {linear_vel}")
...
class CircularTrajectory(Trajectory):
    ...
    def target_pose(self, time):
        ...
        if time <= self.total_time / 2.0:
            ...
            second_half = time - self.total_time / 2.0
            starting = 1/2 * self.angular_acceleration * (self.total_time / 2.0) ** 2

            theta = starting + self.angular_v_max*second_half - 1/2 * self.angular_acceleration * second_half ** 2 # add pi
    ...
    def target_velocity(self, time):
        ...
        if time <= self.total_time / 2.0:
            ...
            second_half = time - self.total_time / 2.0
            starting = 1/2 * self.angular_acceleration * (self.total_time / 2.0) ** 2
            theta = starting + self.angular_v_max*second_half - 1/2 * self.angular_acceleration * second_half ** 2 
            theta_dot = self.angular_v_max - self.angular_acceleration * second_half 
