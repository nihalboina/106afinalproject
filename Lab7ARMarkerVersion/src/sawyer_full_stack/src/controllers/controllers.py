...
class PIDJointVelocityController(Controller):
    ...
    def step_control(self, target_position, target_velocity, target_acceleration):
        ...
        current_pos_error = target_position - current_position
        self.integ_error = self.Kw * self.integ_error + current_pos_error
        current_vel_error = target_velocity - current_velocity
        controller_velocity = target_velocity + np.matmul(self.Kp,current_pos_error) + np.matmul(self.Ki,self.integ_error) + np.matmul(self.Kd,current_vel_error)
        self._limb.set_joint_velocities(joint_array_to_dict(controller_velocity, self._limb))