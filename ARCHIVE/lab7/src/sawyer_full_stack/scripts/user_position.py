...
def map_keyboard(side):
    ...
    while not done and not rospy.is_shutdown():
        go = False
        c = input("Input 7 angles in radians separated by commas: ")
        if c:
            #catch Esc or ctrl-c
            if c in ['\x1b', '\x03']:
                done = True
                ...
                go = True
                angles_str = c.split(',')
                angles = [float(angle) for angle in angles_str]

                cmd_dict = {}
                for joint, angle in zip(joints, angles):
                    cmd_dict[joint] = angle

                print(cmd_dict[joints[0]])
                print(cmd_dict[joints[4]])

        while go:

            limb.set_joint_positions(cmd_dict)
            sleep(0.01)
            go = False

            for joint_name in joints:
                current_position = limb.joint_angle(joint_name)
                desired_position = cmd_dict[joint_name]
                if(abs(current_position - desired_position) > 0.1):
                    go = True
